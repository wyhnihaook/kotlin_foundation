...
/**
 * 描述:自定义Paging适配器
 * 功能介绍:存在footer的情况，要让底部一直展示对应的状态。加载中/加载失败/没有更多数据（当前只能展示加载中和加载失败的状态，缺少没有更多数据的状态）
 * 该类没有任何其他修改，只是把引用给修改了，只是把状态同步给LoadStateAdapter（重点修改对象->修改为LoadStateCustomAdapter）
 * 创建者:翁益亨
 * 创建日期:2023/2/22 9:38
 */
 ...
 */
abstract class PagingDataCustomAdapter<T : Any, VH : RecyclerView.ViewHolder> @JvmOverloads constructor(
    ...
    init {
        ...
        fun considerAllowingStateRestoration() {
            if (stateRestorationPolicy == PREVENT && !userSetRestorationPolicy) {
                this@PagingDataCustomAdapter.stateRestorationPolicy = ALLOW
            }
        }
